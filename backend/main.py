...
def model_provenance():
    return {
        ...
        "regulation_agents": regulation_agent_configuration(),
        "note": "REGIQ separates product recognition from regulatory investigation. Model weights retain their own licenses.",
    }
...
def scan_config():
    ...
    return {
        ...
        "regulatory_catalog_version": catalog.get("catalog_version"),
        "principle": "REGIQ identifies the product, then an investigator LLM screens the full verified regulatory corpus and a verifier LLM challenges the findings. Deterministic category rules are retained only as a fallback/benchmark.",
    }
...
async def scan_image(
    ...
    agent_profile = await investigate_regulation(identification, hf_token_override=x_regiq_hf_token)
    if agent_profile:
    ...
    return {
        ...
        "discovery": {
            ...
            "message": "The investigator works against REGIQ's verified legal corpus. A future source-discovery agent can propose new official acts for verification before they enter that corpus.",
        },
    }
