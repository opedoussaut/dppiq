...
async function runJsonStage(id, label, subtitle, url, body) {
  ...
  try {
    ...
    if (!response.ok || !payload) {
      emitActivity({ id, phase: 'error', label, elapsed_ms: elapsed, message: payload?.detail || `HTTP ${response.status}` })
      throw new Error(payload?.detail || `${label} failed`)
    }
    emitActivity({ id, phase: 'complete', label, elapsed_ms: elapsed, server_elapsed_ms: payload.elapsed_ms })
    return payload
  } catch (error) {
    const elapsed = performance.now() - started
...
async function stagedIntelligence(identification) {
  const investigationPayload = await runJsonStage(
    ...
    'Mapping supported product evidence against the verified EU corpus',
  ...
  const verificationPayload = await runJsonStage(
    ...
    'Challenging applicability findings and unsupported assumptions',
...
function composeScanResult(file, identification, profile, fast, stageMetrics = {}) {
  return {
    ...
    discovery: {
      ...
      message: profile ? 'Staged investigator and verifier completed.' : 'Product identity is ready; intelligence is still pending.',
    },
    fast_scan: {
...
async function interceptScan(input, init = {}) {
  ...
  if (fast?.identification?.status === 'identified') {
    ...
    try {
      ...
      emitActivity({ id: 'dossier', phase: 'complete', label: 'Regulatory dossier ready', elapsed_ms: performance.now() - scanStartedAt })
      const result = composeScanResult(file, identification, staged.regulatory_profile, fast, staged.stage_metrics)
      return new Response(JSON.stringify(result), { status: 200, headers: { 'content-type': 'application/json' } })
    } catch {
      return fallbackDeepScan(input, init, 'Staged intelligence degraded · using full fallback pipeline')
    }
  }
...
window.fetch = function regiqStagedFetch(input, init = {}) {
